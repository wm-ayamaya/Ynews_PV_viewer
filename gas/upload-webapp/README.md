...
flowchart TD
  ...
  V6 --> V7[全データ行を走査し配信日時の最早・最遅を集計]
  V7 --> V8[行数上限で返却 rows のみ切り詰め]
  V8 --> V9[headers・rows・統計メタ等を JSON で返す]
  V9 --> V10[ブラウザ: 表描画・PV 桁区切りと横棒・△▽ ソート]
```
...
- 画面上部に **統計の期間** を大きめの文字で表示します。
  - ファイル名の **`_整理_YYYYMMDD`** から、整理時に指定した **配信開始日**（その日 0:00 以降を対象とした集計である旨）を表示します。手動リネーム等で形式が合わない場合は判別不可のメッセージになります。
  - ファイル内の全データ行から **`配信日時` の最早〜最遅** を算出して併記します（返却行が上限で切られていても、ファイル全体に基づきます）。
  - **掲載記事数**（ファイル内のデータ行数）を表示します。右側に **「※wmapv（動画記事）は含みません。」** の注記があります（Insights エクスポートの前提に合わせた説明）。
  - 続けて **ファイル名**・**更新時刻**（ドライブ上の最終更新）を表示します。
- 表の列順は **配信日時** → **合計PV数** → **記事タイトル**（URL があればタイトルがリンク）。
- **合計PV数**は 3 桁区切りの数値に加え、**現在表示している一覧の合計 PV を 100%** としたときの **各記事の割合** を、セル背景の **横棒** で示します（並び替えのたびに一覧が変わるため、棒の割合も表示中の行だけを基準に再計算されます）。
- **△（昇順） / ▽（降順）** ボタンで配信日時・合計 PV のソート。
- 表領域は選択・コピーを抑止する実装あり（完全な防止ではありません）。
- 一度に返す行数の上限は `Code.gs` の `MAX_VIEW_ROWS`（既定 5000）。上限超過分は返却されませんが、掲載記事数・最早最遅の集計はファイル内の全行を対象にします。

### `loadLatestProcessedArticles` の返却（閲覧用 JSON の要点）

| フィールド | 説明 |
|------------|------|
| `rows` / `rowCount` / `truncated` | 表示用データ行と件数、切り詰め有無 |
| `totalArticlesInFile` | ヘッダ除くファイル内のデータ行総数 |
| `statsFilterFromIso` | ファイル名から解釈した開始日 `YYYY-MM-DD`（無ければ `null`） |
| `statsDispatchMin` / `statsDispatchMax` | ファイル全体の配信日時 最早・最遅（表示用に整形済み） |
| `fileName` / `lastUpdated` / `headers` | 従来どおり |
